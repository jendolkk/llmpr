...
for subdir in os.walk('D:\\wyh\\data\\2016\\desc'):
  print(subdir)
  if len(subdir[2]) > 0:
    subdir[2].sort(key=lambda x: int(x[:-5]))
  ...
  for i, img in enumerate(subdir[2]):
    ...
    if int(img[27:-4]) > 100:
      ...
      response = chat(model='captioner', messages=[
        {
          'role': 'user',
          'content': 'Analyze the provided patent image and generate a concise description focusing on the unique features that distinguish it from other patents without speculative interpretation, such as key structural, functional, or design elements. Ensuring clarity and precision in identifying the patent\'s distinctive characteristics. ATTENTION: NO MORE THAN 450 WORDS',
